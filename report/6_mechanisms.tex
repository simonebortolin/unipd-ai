...
Code of CRSD is illustrated the algorithm~\ref{alg:crsd}.

Constrained rank value (CRV) mechanism it is a variant of the CRSD one that uses a rank function.
It is based on \(\hat {I} = ( F , C , P_r , \pi  ,v, \alpha )\) where \(v\) is a rank function express ranks assigning a values between 0 and 1 to positions in preference orders that is monotonically decreasing~\cite{olbergml}.
Code of CRV is illustrated the algorithm~\ref{alg:crv}.

The \(\pi\)-machine learning is Admissible for COM because is algorithm to calculate a value starting from another value and even if it is wrong it does not cause loss of life.
